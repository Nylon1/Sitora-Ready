...
export type GovernanceStatus = 'Approved' | 'Draft';
...
export const findRegistryTreatment = (treatment: string) => {
  const value = treatment.trim().toLowerCase();
  return treatmentRegistry.find((item) =>
    item.label.toLowerCase() === value || item.aliases.some((alias) => value.includes(alias)),
  );
};
